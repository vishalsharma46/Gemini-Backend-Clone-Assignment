...
from .. import models
from ..schemas import SignupRequest, SendOtpRequest, VerifyOtpRequest, ChangePasswordRequest, TokenResponse, UserOut
from ..auth import issue_otp, verify_otp, create_access_token, hash_password, verify_password
...
def signup(body: SignupIn, db: Session = Depends(get_db)):
    ...
    if not user:
        user = models.User(mobile=body.mobile, tier="basic")
        db.add(user)
    else:
